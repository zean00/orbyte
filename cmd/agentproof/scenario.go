	...
	return mustJSONString([]map[string]any{
		...
		},
		{
			...
			"required_final_facts": []string{
				...
				"Verified ticket id and ticket number from MCP output when a new ticket is created.",
			},
			"required_draft_outputs": []string{
				"ticket id when a new complaint ticket is created",
				"ticket number when a new complaint ticket is created",
			...
			"guardrails": []string{
				...
				"Do not treat a CRM-... ticket number returned by create_ticket as the same thing as the internal crm_ticket:... record id.",
				"Do not restrict the first recovery search after create_ticket to status=open when the newly created ticket may still be in status new or another initial state.",
				"Do not finish a newly created complaint ticket workflow until crm.ticket.get or crm.ticket.search has confirmed both a stable ticket id and ticket number from MCP output.",
				"Do not present a narrative-only success answer for a new ticket when the workflow has not confirmed the ticket reference from tool output.",
			},
			"success_checks": []string{
				"Final answer clearly states whether a new complaint ticket was created or an existing one should be reused.",
				"When a new ticket is created, the answer includes the confirmed ticket id, ticket number, selected queue, and severity context.",
				"The workflow is not complete for a newly created ticket unless the ticket reference was confirmed from MCP output.",
			},
			"pitfalls": []string{
