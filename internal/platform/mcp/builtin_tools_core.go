...
func (s *Server) appendBuiltInCoreToolRegistrations(registry []builtInToolRegistration) []builtInToolRegistration {
	registry = append(registry,
		mustBuiltInToolRegistration((*Server).toolsListMeta, builtInTool{
			...
			description: "Fallback step only when no playbook matches. List lightweight summaries of discoverable MCP tools after playbook discovery fails. For details, pass ALL candidate IDs to tools.describe in a single call.",
		...
		mustBuiltInToolRegistration((*Server).toolsSearchMeta, builtInTool{
			name:        "tools.search",
			title:       "Search Tools",
			description: "Fallback step only when no playbook matches. Search discoverable MCP tools by title, description, domains, and labels after playbook discovery fails. Pass ALL candidate IDs to tools.describe in a single call.",
			permission:  "module.read",
		...
		mustBuiltInToolRegistration((*Server).toolsDescribeMeta, builtInTool{
			...
			description: "Only use when no playbook matches your use case. Get detailed tool descriptions, schemas, and contracts in bulk. Always pass ALL candidate tool IDs from tools/search in a single call — do not describe tools one at a time.",
			permission:  "module.read",
		...
		mustBuiltInToolRegistration((*Server).playbooksListMeta, builtInTool{
			name:        "skills.list",
			title:       "List Skills",
			description: "Required first step for workflow-like business tasks in minimal mode. List workflow skills to find one matching the current use case before searching individual tools. Only fall back to tools.search or tools.list when no skill fits.",
			permission:  "module.read",
		...
		mustBuiltInToolRegistration((*Server).playbooksSearchMeta, builtInTool{
			name:        "skills.search",
			title:       "Search Skills",
			description: "Required first step for workflow-like business tasks in minimal mode. Search skills by use case or intent, try to find a matching skill first before searching individual tools, then call skills.describe and follow the matched workflow.",
			permission:  "module.read",
		...
		mustBuiltInToolRegistration((*Server).playbooksListMeta, builtInTool{
			...
			description: "Required first step for workflow-like business tasks in minimal mode. List workflow playbooks to find one matching the current use case before searching individual tools. Only fall back to tools.search or tools.list when no playbook fits.",
			permission:  "module.read",
			inputSchema: map[string]any{"type": "object", "properties": map[string]any{"domain": map[string]any{"type": "string"}, "label": map[string]any{"type": "string"}, "limit": map[string]any{"type": "integer"}}},
		}),
		mustBuiltInToolRegistration((*Server).playbooksSearchMeta, builtInTool{
			...
			description: "Required first step for workflow-like business tasks in minimal mode. Search playbooks by use case or intent, try to find a matching playbook first before searching individual tools, then call playbooks.describe and follow the matched workflow.",
			permission:  "module.read",
			inputSchema: map[string]any{"type": "object", "properties": map[string]any{"query": map[string]any{"type": "string"}, "domain": map[string]any{"type": "string"}, "label": map[string]any{"type": "string"}, "limit": map[string]any{"type": "integer"}}},
		}),
		mustBuiltInToolRegistration((*Server).playbooksDescribeMeta, builtInTool{
