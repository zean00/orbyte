...
func (s *Server) appendBuiltInCoreToolRegistrations(registry []builtInToolRegistration) []builtInToolRegistration {
	registry = append(registry,
		mustBuiltInToolRegistration((*Server).toolsListMeta, builtInTool{
			...
			description: "Only use when no playbook matches your use case. List lightweight summaries of discoverable MCP tools. For details, pass ALL candidate IDs to tools.describe in a single call.",
		...
		mustBuiltInToolRegistration((*Server).toolsSearchMeta, builtInTool{
			name:        "tools.search",
			title:       "Search Tools",
			description: "Only use when no playbook matches your use case. Search discoverable MCP tools by title, description, domains, and labels. Pass ALL candidate IDs to tools.describe in a single call.",
			permission:  "module.read",
		...
		mustBuiltInToolRegistration((*Server).playbooksListMeta, builtInTool{
			...
			description: "First step: list workflow playbooks to find one matching your current use case. If a playbook matches, follow its tool sequence. Only fall back to tools.search when no playbook fits.",
			permission:  "module.read",
		...
		mustBuiltInToolRegistration((*Server).playbooksSearchMeta, builtInTool{
			...
			description: "First step: search playbooks by your use case keywords. If a playbook matches, call playbooks.describe and follow its tool sequence. Only fall back to tools.search when no playbook fits.",
			permission:  "module.read",
