...
func TestPostgresRepositorySystemAndSubmissionLifecycle(t *testing.T) {
	...
	systemRows := sqlmock.NewRows([]string{"system_key", "name", "status", "adapter_key", "connector_key", "description", "settings_json", "created_at", "updated_at"}).
		AddRow("http_bridge", "HTTP Bridge", "active", "http", "http", "Bridge", []byte(`{"url":"https://example.test"}`), now, now)
	mock.ExpectQuery(regexp.QuoteMeta("SELECT system_key, name, status, adapter_key, COALESCE(connector_key,''), COALESCE(description,''), settings_json, created_at, updated_at FROM integration_systems WHERE system_key = $1")).
	...
	submissionRows := sqlmock.NewRows([]string{"submission_id", "external_system_key", "endpoint_key", "contract_key", "contract_version", "intent", "mode", "idempotency_key", "operation_type", "status", "document_id", "correlation_id", "external_reference", "attempt_count", "last_error", "payload_json", "result_json", "processed_at", "created_at", "updated_at"}).
		AddRow("sub-1", "http_bridge", "", "document.submit", 1, "command", "sync", "", "push_document", "queued", "doc-1", "corr-1", "", 1, "", []byte(`{"title":"hello"}`), []byte(`{}`), nil, now, now)
	mock.ExpectQuery(regexp.QuoteMeta("SELECT submission_id, external_system_key, COALESCE(endpoint_key,''), COALESCE(contract_key,''), contract_version, COALESCE(intent,''), COALESCE(mode,''), COALESCE(idempotency_key,''), operation_type, status, COALESCE(document_id,''), COALESCE(correlation_id,''), COALESCE(external_reference,''), attempt_count, COALESCE(last_error,''), payload_json, result_json, processed_at, created_at, updated_at FROM integration_submissions WHERE submission_id = $1")).
