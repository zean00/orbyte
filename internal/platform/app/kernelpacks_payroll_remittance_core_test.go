...
func TestPayrollRemittanceManifestExposesExpectedArtifacts(t *testing.T) {
	manifest := payrollRemittanceCoreKernelPackManifest()

	if manifest.Key != "payroll_remittance_core" {
	...
	if len(manifest.Models) != 4 {
		t.Fatalf("expected 4 remittance models, got %d", len(manifest.Models))
	}
}

func TestPayrollRemittanceRoleTemplateIncludesWorkflowPermissions(t *testing.T) {
	manifest := payrollRemittanceCoreKernelPackManifest()
	if len(manifest.Security.RoleTemplates) == 0 {
		t.Fatal("expected role templates")
	}
	role := manifest.Security.RoleTemplates[0]
	required := []string{"remittance.read", "document.create", "document.submit", "document.approve", "remittance_authority.list"}
