	...
	return module.Manifest{
		...
		Models: []model.Definition{
			...
			leavePolicyModelDefinition("leave_accrual_run", "Leave Accrual Run", []model.FieldDefinition{
				...
				{Key: "run_status", Type: "string", DefaultValue: "pending"},
			...
			RoleTemplates: []module.RoleTemplateDefinition{
				{
					...
					PermissionKeys: []string{
						...
						"attendance.read", "attendance.update", "attendance.leave_inbox.read", "attendance.amend", "attendance.cancel",
					},
				},
				{
		...
		SelfService: module.SelfServiceDefinition{
			APIs: []module.SelfServiceAPIDefinition{
				...
				{Key: "leave.self_service.requests.amend", Title: "Amend My Leave Request", TitleI18n: localize("Amend My Leave Request", "Ubah Permintaan Cuti Saya"), Method: "POST", RoutePath: "/ui/self-service/leave/requests/{requestID}/amend", HandlerKind: "custom", ModelKey: "leave_request", RequiredPermissions: []string{"leave.self_service.write"}, AudienceKinds: []string{"employee"}, RequestContractKey: "leave.self_service.request.update", ResponseContractKey: "leave.self_service.request.detail", Idempotent: true},
...
func leavePolicyViews() []module.ViewDefinition {
	return []module.ViewDefinition{
		...
		commercialModelListView("leave.accrual_runs.list", "Leave Accrual Runs", "leave_accrual_run", []module.ColumnDefinition{{Key: "code", Label: "Code", Path: "values.code"}, {Key: "run_mode", Label: "Mode", Path: "values.run_mode"}, {Key: "run_status", Label: "Run Status", Path: "values.run_status"}, {Key: "status", Label: "Status", Path: "values.status"}}, []string{"pending", "processing", "completed", "failed", "cancelled", "active", "inactive"}),
...
func leavePolicyFields(modelKey string, form bool) []module.FieldDefinition {
	defs := map[string][]module.FieldDefinition{
		...
		"leave_accrual_run": {
			...
			{Key: "run_status", Label: "Run Status", Path: "values.run_status", Type: "string", Widget: widgetForForm(form, "select"), Options: []string{"pending", "processing", "completed", "failed", "cancelled"}},
