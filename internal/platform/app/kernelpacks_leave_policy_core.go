...
func leavePolicyFields(modelKey string, form bool) []module.FieldDefinition {
	defs := map[string][]module.FieldDefinition{
		...
		"leave_entitlement_rule": {
			...
			{Key: "carry_forward_expiry_rule", Label: "Carry Forward Expiry Rule", Path: "values.carry_forward_expiry_rule", Type: "string", Widget: widgetForForm(form, "text")},
		...
		"leave_balance_entry": {
			...
			{Key: "entry_type", Label: "Entry Type", Path: "values.entry_type", Type: "string", Widget: widgetForForm(form, "select"), Options: []string{"opening", "annual_grant", "monthly_accrual", "carry_forward", "expiry", "reservation", "release", "consumption", "adjustment"}},
