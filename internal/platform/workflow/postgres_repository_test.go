...
func TestPostgresRepositoryWorkflowLifecycle(t *testing.T) {
	...
	taskRows := sqlmock.NewRows([]string{"task_id", "workflow_key", "target_type", "target_id", "task_type", "status", "created_at"}).AddRow("task:1", "generic_request_flow", "document", "doc1", "review", "open", now)
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT task_id, workflow_key, target_type, target_id, task_type, status, created_at FROM workflow_tasks")).WillReturnRows(taskRows)
	if len(repo.ListTasks()) != 1 {
		t.Fatal("expected listed workflow task")
	}

	approvalRows := sqlmock.NewRows([]string{"approval_id", "workflow_key", "target_type", "target_id", "status", "requested_at"}).AddRow("approval:1", "generic_request_flow", "document", "doc1", "pending", now)
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT approval_id, workflow_key, target_type, target_id, status, requested_at FROM workflow_approvals")).WillReturnRows(approvalRows)
	if len(repo.ListApprovals()) != 1 {
		t.Fatal("expected listed workflow approval")
	}

	mock.ExpectExec(regexp.QuoteMeta("UPDATE workflow_tasks SET status = $1 WHERE task_id = $2")).WillReturnResult(sqlmock.NewResult(1, 1))
	if err := repo.UpdateTaskStatus("task:1", "completed"); err != nil {
		t.Fatalf("update task failed: %v", err)
	}
	mock.ExpectExec(regexp.QuoteMeta("UPDATE workflow_approvals SET status = $1 WHERE approval_id = $2")).WillReturnResult(sqlmock.NewResult(1, 1))
	if err := repo.UpdateApprovalStatus("approval:1", "approved"); err != nil {
