...
func (r *PostgresRepository) Save(event Event) error {
	...
	const query = `
		INSERT INTO audit_events (
			...
			actor_kind, from_state, to_state, organization_id, location_id, operating_unit_id,
			request_id, occurred_at, change_summary_json, metadata_json, correlation_id
		) VALUES ($1,$2,$3,$4,$5,NULLIF($6,''),NULLIF($7,''),NULLIF($8,''),NULLIF($9,''),NULLIF($10,''),NULLIF($11,''),NULLIF($12,''),$13,$14,$15,NULLIF($16,''))`
...
func (r *PostgresRepository) Query(filter Query) []Event {
	const query = `
		SELECT audit_event_id, action, target_type, target_id, actor_id,
			COALESCE(actor_kind,''), COALESCE(from_state,''), COALESCE(to_state,''),
		...
		WHERE ($1 = '' OR target_type = $1)
		  ...
		  AND ($8 = '' OR location_id = $8)
		  AND ($9 = '' OR operating_unit_id = $9)
		  AND ($10::timestamptz IS NULL OR occurred_at >= $10)
		  AND ($11::timestamptz IS NULL OR occurred_at <= $11)
		ORDER BY occurred_at ASC`
	rows, err := r.db.QueryContext(context.Background(), query,
		filter.TargetType, filter.TargetID, filter.ActorID, filter.ActorKind, filter.Action,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Action, &item.TargetType, &item.TargetID, &item.ActorID, &item.ActorKind, &item.FromState, &item.ToState, &item.OrganizationID, &item.LocationID, &item.OperatingUnitID, &item.RequestID, &item.OccurredAt, &changeSummary, &metadata, &item.CorrelationID); err != nil {
			continue
		}
