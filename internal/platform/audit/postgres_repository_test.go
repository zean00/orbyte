...
func TestPostgresAuditRepository(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"audit_event_id", "action", "target_type", "target_id", "actor_id", "actor_kind", "on_behalf_of_user_id", "delegation_grant_id", "from_state", "to_state", "organization_id", "location_id", "operating_unit_id", "request_id", "occurred_at", "change_summary_json", "metadata_json", "correlation_id"}).AddRow("a1", "submit", "document", "d1", "u1", "user", "", "", "draft", "submitted", "org_default", "loc_hq", "", "req-1", now, []byte(`{"fields":["body.payload"]}`), []byte(`{"x":1}`), "c1")
