...
func TestUILeaveAmendmentRoutesEnforcePermissionsAndUpdateInPlace(t *testing.T) {
	h := newTestHarness(t)
	for _, permissionKey := range []string{"leave.self_service.read", "leave.self_service.write", "attendance.read", "attendance.leave_inbox.read", "attendance.approve", "attendance.reject", "attendance.cancel", "attendance.amend"} {
		if err := h.ident.UpsertPermission(identity.Permission{Key: permissionKey, Module: "leave_policy_core", Action: "use", Resource: "leave_self_service"}); err != nil {
			t.Fatalf("upsert permission %s: %v", permissionKey, err)
	...
	absenceCode, _ := h.models.Create("absence_code", "user_admin", map[string]any{"code": "ANNUAL-AMD", "name": "Annual Leave", "category": "leave", "status": "active"})
	policyRecord, _ := h.models.Create("leave_policy", "user_admin", map[string]any{"code": "LP-AMD", "name": "Amend Policy", "absence_code_id": absenceCode.ID, "paid_leave": false, "requires_balance": false, "allows_half_day": true, "status": "active"})
	approvalPolicy, _ := h.models.Create("approval_policy", "user_admin", map[string]any{"code": "LEAVE-AMD", "name": "Leave Approval Policy", "document_type": "leave_request", "workflow_key": "leave_request_flow", "department_id": "dept_leave", "action": "submit", "status": "active"})
	if _, err := h.models.Create("approval_policy_stage", "user_admin", map[string]any{"policy_id": approvalPolicy.ID, "stage_key": "mgr", "sequence": 1, "assignment_strategy": "explicit_user", "explicit_user_id": "user_admin", "status": "active"}); err != nil {
		t.Fatalf("create approval stage: %v", err)
	}
	if _, err := h.models.Create("leave_entitlement_rule", "user_admin", map[string]any{"leave_policy_id": policyRecord.ID, "grant_mode": "annual_grant", "annual_entitlement_days": 8.0, "status": "active"}); err != nil {
		t.Fatalf("create entitlement rule: %v", err)
	...
	amendBody, _ := json.Marshal(map[string]any{
		"leave_policy_id": policyRecord.ID,
		"start_date":      "2099-02-03",
		"end_date":        "2099-02-03",
		"reason":          "shifted schedule",
	})
	selfAmend := h.request(http.MethodPost, "/ui/self-service/leave/requests/"+requestID+"/amend", amendBody, true)
	if selfAmend.Code != http.StatusOK {
		t.Fatalf("self-service amend failed: %d body=%s", selfAmend.Code, selfAmend.Body.String())
	}
	var amended map[string]any
	_ = json.Unmarshal(selfAmend.Body.Bytes(), &amended)
	amendedRecord := amended["record"].(map[string]any)
	if amendedRecord["id"] != requestID {
		t.Fatalf("expected amend to update request in place, got %v", amendedRecord["id"])
	}
	if amendedRecord["start_date"] != "2099-02-03" {
		t.Fatalf("expected amended start date, got %+v", amendedRecord)
	}

	managerAmend := h.request(http.MethodPost, "/ui/attendance/leave-requests/"+requestID+"/amend", amendBody, true)
	if managerAmend.Code != http.StatusForbidden {
		t.Fatalf("expected manager amend to require attendance.amend, got %d body=%s", managerAmend.Code, managerAmend.Body.String())
	}
	if err := h.ident.GrantRolePermission(identity.RolePermission{RoleID: "role_admin", PermissionKey: "attendance.amend"}); err != nil {
		t.Fatalf("grant attendance.amend: %v", err)
	}
	approve := h.request(http.MethodPost, "/ui/attendance/leave-requests/"+requestID+"/approve", nil, true)
	if approve.Code != http.StatusOK {
		t.Fatalf("approve leave request failed: %d body=%s", approve.Code, approve.Body.String())
	...
	managerAmend = h.request(http.MethodPost, "/ui/attendance/leave-requests/"+requestID+"/amend", amendBody, true)
	if managerAmend.Code != http.StatusOK {
		t.Fatalf("manager amend failed: %d body=%s", managerAmend.Code, managerAmend.Body.String())
	}
}

func TestUIManagerAmendRouteDoesNotRequireInboxVisibilityAfterMutation(t *testing.T) {
	h := newTestHarness(t)
	for _, permissionKey := range []string{"attendance.amend"} {
		if err := h.ident.UpsertPermission(identity.Permission{Key: permissionKey, Module: "leave_policy_core", Action: "use", Resource: "leave_self_service"}); err != nil {
			t.Fatalf("upsert permission %s: %v", permissionKey, err)
		}
		if err := h.ident.GrantRolePermission(identity.RolePermission{RoleID: "role_admin", PermissionKey: permissionKey}); err != nil {
	...
	absenceCode, _ := h.models.Create("absence_code", "user_admin", map[string]any{"code": "ANNUAL-MGR", "name": "Annual Leave", "category": "leave", "status": "active"})
	policyRecord, _ := h.models.Create("leave_policy", "user_admin", map[string]any{"code": "LP-MGR", "name": "Manager Amend Policy", "absence_code_id": absenceCode.ID, "paid_leave": true, "requires_balance": false, "allows_half_day": true, "status": "active"})
	request, err := h.models.Create("leave_request", "user_admin", map[string]any{
		"employee_id":                employee.ID,
		"organization_id":            "org_default",
		"location_id":                "loc_hq",
		"absence_code_id":            absenceCode.ID,
		...
		"self_service_actor_user_id": "employee_user",
		"status":                     "active",
	})
	if err != nil {
	...
	resp := h.request(http.MethodPost, "/ui/attendance/leave-requests/"+request.ID+"/amend", amendBody, true)
	if resp.Code != http.StatusOK {
		t.Fatalf("expected manager amend success without inbox visibility, got %d body=%s", resp.Code, resp.Body.String())
	}
}
