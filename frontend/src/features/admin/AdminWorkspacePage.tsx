...
export default function AdminWorkspacePage() {
  ...
  const title = useMemo(() => {
    return (
      routes.find((item) => item.path === path)?.label || titleForPath(path)
    );
  }, [path, routes]);
...
function AdminContent({
  ...
  if (path === "/mcp") {
    return <MCPAdminPage payload={payload} />;
  }

  if (path === "/acp") {
    return <ACPAdminPage payload={payload} />;
  }
  ...
  if (path === "/observability") {
    ...
    const logEvents = asItems(
      payload?.log_events as Record<string, unknown> | null,
    );
    const domainEvents = asItems(
      payload?.domain_events as Record<string, unknown> | null,
    );
    const statuses = asItems(
      payload?.contract_status as Record<string, unknown> | null,
    );
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <SummaryCard
            label="Domain Events"
            value={String(domainEvents.length)}
          />
  ...
  return (
    <ValueCard
      label="Raw payload"
      value={payload ?? bootstrapSummary(path, bootstrap)}
    />
  );
}
...
function endpointForPath(path: string): string {
  ...
  if (normalizedPath.startsWith("/modules/")) {
    ...
    return moduleKey
      ? `/admin/api/modules/${encodeURIComponent(moduleKey)}/console`
      : "";
  }
...
function buildClientModuleDependencyGraph(
...
function TemplateListPage({ rows }: { rows: Array<Record<string, unknown>> }) {
  ...
  const targetOptions =
    draft.targetKind === "report"
      ? targetCatalog.reports || []
  ...
  async function deleteTemplate(row: Record<string, unknown>) {
    ...
    try {
      await mutateJson(
        `/admin/api/templates/definitions/${encodeURIComponent(key)}`,
        {
          method: "DELETE",
        },
      );
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-line bg-accent-soft/60 p-4 text-sm text-body">
        Select a template to open the visual designer, preview drafts, and use
        advanced body or style editing.
      </div>
      <section className="rounded-xl border border-line bg-surface p-4 dark:bg-ink/60">
        <div className="mb-3 text-sm font-semibold text-body">
          Create New Template
      ...
      <DataGrid
        ...
        onAction={(row) =>
          navigate(
            `/templates/designer?key=${encodeURIComponent(String(resolvePath(row, "key") || ""))}`,
          )
        }
...
function WorkflowListPage({ rows }: { rows: Array<Record<string, unknown>> }) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-line bg-accent-soft/60 p-4 text-sm text-body">
        Select a workflow to open the visual designer, manage versions, validate
        transitions, and simulate routing.
      </div>
      <section className="rounded-xl border border-line bg-surface p-4 dark:bg-ink/60">
        <div className="mb-3 text-sm font-semibold text-body">
          Create New Workflow
        ...
        {message ? (
          <div className="mt-3 text-sm text-body">{message}</div>
        ) : null}
      </section>
      <DataGrid
        ...
        onAction={(row) =>
          navigate(
            `/workflows/designer?key=${encodeURIComponent(String(resolvePath(row, "key") || ""))}`,
          )
        }
        secondaryActionLabel="Delete"
